        ...
        <form onSubmit={handleCreate} className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <select value={form.source_warehouse_id} onChange={(e) => { setForm({ ...form, source_warehouse_id: e.target.value }); loadLocations(e.target.value, setSourceLocations); }} className={inputCls}>
            ...
            <div>
              ...
              <select value={form.dest_warehouse_id} onChange={(e) => { setForm({ ...form, dest_warehouse_id: e.target.value }); loadLocations(e.target.value, setDestLocations); }} className={inputCls}>
                <option value="">Select</option>
                {(warehouses || []).map((w) => <option key={w.id} value={w.id}>{w.name}</option>)}
              </select>
            </div>
